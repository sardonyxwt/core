import { deepFreeze } from './util/object.utils';
 ...
 * @returns {string}
 */
const createUniqueIdGenerator = (prefix: string): Generator => {
    let index = 0;
    return () => `${prefix}:${++index}`;
};
    ...
    publish<T = unknown>(eventName: string, data?: T) {
        const eventDispatcher = () => {
            if (
                ...
                deepFreeze(data as Record<string, unknown>);
            }
...
export function createEventBus(config: EventBusConfig = {}): Eventbus {
    const { name = generateEventBusName(), isImmutabilityEnabled = false } =
        config;
